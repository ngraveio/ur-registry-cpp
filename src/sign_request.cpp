...
size_t SignRequest::getMapSize() const
{
    ...
    if (getRequestID().has_value())
        ++size;
    if (getDerivationPath().has_value())
        ++size;
    if (getOrigin().has_value())
        ++size;
    if (getTxtype().has_value())
        ++size;
    if (std::holds_alternative<std::monostate>(m_address) == false)
        ++size;
    if (getIntent().has_value())
        ++size;
...
void SignRequest::fromMap(CborValue *cborMap)
{
    ...
    while (!cbor_value_at_end(&localDecoder))
    {
        ...
        if (key == Key::SIGN_DATA || key == Key::REQUEST_ID || key == Key::ORIGIN || key == Key::TXTYPE || key == Key::ADDRESS)
        {
            err = cbor_value_advance(&localDecoder);
            checkCborError(err, "Failed to advance");
        }
